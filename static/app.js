...
function showGps() {
  ...
  // координаты: принудительно с raw.githubusercontent (мимо SW и CDN Pages)
  const GPS_RAW =
  ...
  async function fetchGpsConfig() {
    ...
    // 1) raw — другой origin → Service Worker не перехватывает
    try {
      const r = await fetch(GPS_RAW + bust, {
        cache: 'no-store',
        mode: 'cors',
    ...
    // 2) GitHub API — тоже другой origin, обычно свежее
    try {
      const r = await fetch(GPS_API + bust, {
        cache: 'no-store',
        mode: 'cors',
        headers: {
      ...
      if (r.ok) {
        ...
        if (ct.includes('json') && !ct.includes('raw')) {
    ...
    // 3) локальный config.json (если вдруг offline к raw)
    try {
      const r = await fetch('config.json' + bust, { cache: 'no-store' });
      if (r.ok) return await r.json();
    } catch (_) {}
    return null;
  }
  ...
  async function pollGps() {
    try {
      ...
      let age = null;
      if (j.updated) {
        // updated без таймзоны → как UTC, иначе телефон врёт age
        let u = String(j.updated).trim();
        if (u && !/Z|[+-]\d\d:?:\d\d$/.test(u)) u += 'Z';
        const ts = Date.parse(u);
        if (!isNaN(ts)) age = Math.max(0, Math.round((Date.now() - ts) / 1000));
      }
      if (j.age_s != null && !isNaN(Number(j.age_s))) {
        age = Number(j.age_s);
      }
      // ONLINE если есть координаты и они не старше 2 мин (флаг online с сервера не обязателен)
      const online = !!(hasFix && (age == null || age < 120));
      setHud(online, hasFix ? lat : null, hasFix ? lon : null, age);
      if (hasFix) pushPos(lat, lon);
    } catch (_) {
