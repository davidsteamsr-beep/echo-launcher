...
function showGps() {
  ...
  // координаты: несколько источников, каждый раз новый URL (анти-кеш)
  const GPS_SOURCES = [
    // statically.io обычно меньше кеширует raw
    'https://cdn.statically.io/gh/davidsteamsr-beep/echo-launcher/main/config.json',
    'https://raw.githubusercontent.com/davidsteamsr-beep/echo-launcher/main/config.json',
    'https://api.github.com/repos/davidsteamsr-beep/echo-launcher/contents/config.json',
    'config.json',
  ];
  let _lastLat = null;
  let _lastLon = null;
  ...
  async function fetchGpsConfig() {
    const bust = 't=' + Date.now() + '&n=' + Math.random().toString(36).slice(2);
    for (const base of GPS_SOURCES) {
      try {
        const isApi = base.indexOf('api.github.com') !== -1;
        const url = base + (base.indexOf('?') >= 0 ? '&' : '?') + bust;
        const headers = {
          'Cache-Control': 'no-cache, no-store, must-revalidate',
          Pragma: 'no-cache',
        };
        if (isApi) headers.Accept = 'application/vnd.github.raw+json';
        const r = await fetch(url, { cache: 'reload', mode: 'cors', headers });
        if (!r.ok) continue;
        if (isApi) {
          const ct = (r.headers.get('content-type') || '').toLowerCase();
          // raw json body
          if (ct.includes('application/json') && ct.includes('github')) {
  ...
  async function pollGps() {
    try {
      const j = await fetchGpsConfig();
      if (!j) throw new Error('cfg');
      const lat = j.lat != null && j.lat !== '' ? Number(j.lat) : null;
      const lon = j.lon != null && j.lon !== '' ? Number(j.lon) : null;
      const hasFix = lat != null && lon != null && !isNaN(lat) && !isNaN(lon);
      let age = null;
      if (j.updated) {
        let u = String(j.updated).trim();
        if (u && !/Z|[+-]\d{2}:?\d{2}$/.test(u)) u += 'Z';
        const ts = Date.parse(u);
        if (!isNaN(ts)) age = Math.max(0, Math.round((Date.now() - ts) / 1000));
      }
      if (j.age_s != null && !isNaN(Number(j.age_s))) age = Number(j.age_s);
      const online = !!(hasFix && (age == null || age < 180));
      setHud(online, hasFix ? lat : null, hasFix ? lon : null, age);
      // двигаем метку всегда при валидном фиксе (даже если числа те же — карта живая)
      if (hasFix) {
        const changed =
          _lastLat !== lat || _lastLon !== lon || _lastUpdated !== String(j.updated || '');
        _lastLat = lat;
        _lastLon = lon;
        _lastUpdated = String(j.updated || '');
        pushPos(lat, lon);
        if (changed) {
          try {
            console.log('[ECHO GPS]', lat, lon, 'age=' + age, j.updated);
          } catch (_) {}
        }
      }
    } catch (_) {
  ...
  pollGps();
  window.__echoGpsPoll = setInterval(pollGps, 3000);
}
